...
";			
	...
	private static void CreateStaticTypeIdFiles(SourceProductionContext context, EnumDefinition definition, IEnumerable<DiscoveredEnumMember> relevantDiscoveredMembers)
	{
		...
		foreach (var member in partialMembers)
		{
			...
			var enumFullTypeName = $"{definition.Namespace}.{definition.BaseClassName}.{definition.Name}";
...
");
			...
			code.AppendLine($@"
{member.Definition} {member.Name} : IHasStaticTypeId<{enumFullTypeName}>
{{
	public static {enumFullTypeName} StaticTypeId => {enumFullTypeName}.GetSingleMember(""{member.Name}"");
