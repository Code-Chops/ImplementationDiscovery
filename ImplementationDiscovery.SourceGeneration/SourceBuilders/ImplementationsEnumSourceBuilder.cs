...
#pragma warning restore CS0168
	...
	private static void CreateEnumFile(SourceProductionContext context, EnumDefinition definition, List<DiscoveredEnumMember> members, AnalyzerConfigOptionsProvider configOptionsProvider)
	{
		...
		var longestConcreteTypeNameLength = members
			.Select(member => member.GetConcreteTypeName(definition))
			.OrderByDescending(name => name.Length)
			.FirstOrDefault()?.Length ?? 0;
...
");
	...
	/// </summary>");
				...
				var outlineSpaces = new String(' ', longestConcreteTypeNameLength - member.GetConcreteTypeName(definition).Length + longestMemberNameLength - member.GetSimpleName(definition).Length);

				var typeName = member.GetConcreteTypeName(definition);
				code.Append(@$"
	{member.Accessibility} static {typeName} {member.GetSimpleName(definition)} {{ get; }} {outlineSpaces}= ({typeName})CreateMember(new DiscoveredObject<{definition.BaseTypeName}>(typeof({member.Value}))).Value.UninitializedInstance;
");
			}
